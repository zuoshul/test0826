...
void MapTileGraphicsObject::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    if (_tile != 0)
    {
        painter->drawPixmap(this->boundingRect().toRect(),*_tile);
    }
    else
    {
        QString string;
        ...
        painter->drawText(this->boundingRect(),string,QTextOption(Qt::AlignCenter));
    }
}
...
void MapTileGraphicsObject::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event)
{
    //必须只能在这里处理在view处理会导致异常
    Q_UNUSED(event);
    emit zoomIn();
}
